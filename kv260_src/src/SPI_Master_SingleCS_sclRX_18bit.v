...
module SPI_Master_SingleCS_sclRX_18bit
  ...
  (
   ...
   output [BIT_PER_TRANSFER-1:0] o_RX_Byte,   // Byte received on MISO
  ...
  // Instantiate Master
  SPI_Master_sclRX_18bit
    #(.SPI_MODE(SPI_MODE),
      .CLKS_PER_HALF_BIT(CLKS_PER_HALF_BIT),
      .BIT_PER_TRANSFER(BIT_PER_TRANSFER)
      ) SPI_Master_Inst
   (
   // Control/Data Signals,
   .i_Rst_L(i_Rst_L),     // FPGA Reset
   .i_Clk(i_Clk),         // FPGA Clock
   .i_sclRX(i_sclRX),     // delayd scl for RX byte  
   ...
   //DEBUG*-------------
   .debug_RXcounter(debug_RXcounter),
   .debug_TXcounter(debug_TXcounter)
   );
      ...
      TRANSFER:
        begin
          ...
          begin
            ...
            begin
              if(i_TX_DV) begin
                r_SM_CS             <= TRANSFER;   //michi removed CS_INACTIVE
                r_CS_n  <= 1'b0;
              end else begin
                r_SM_CS             <= IDLE;   //michi removed CS_INACTIVE
